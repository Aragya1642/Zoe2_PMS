...
Usage: python can_parser.py <logfile>
"""
...
def process_message(msg):
    """
    Called for every parsed CAN message.
    ...
    # Example: print a one-line summary per message
    print(f"[{t}] ID={msg['can_id_hex']}  func=0x{fc:02X}  mod={msg['module_num']}  data={msg['data_hex']}")
    ...
    if not msg["can_id_hex"] == "0400FF80":
        ...
        if fc == 0x00:      # Cell Voltages
            ...
            print(f"    Total voltage: {sum_v}mV  Current: {curr}mA  SOC: {soc}%  Life: {life}")
            ...
            # heat_curr = d[7]
            print(f"    Power: {power}W  MOS Temp: {mos_temp}°C")
            ...
            print(f"    Max Voltage: {max_v}mV  Max Voltage Cell: {max_v_num}  Min Voltage: {min_v}mV  Min Voltage Cell: {min_v_num}  Diff Voltage: {diff_v}mV")
            ...
            min_temp_num = d[3]
            diff_temp = d[4]
            print(f"    Max Temp: {max_temp}°C  Max Temp Unit: {max_temp_num}  Min Temp: {min_temp}°C  Min Temp Unit: {min_temp_num}  Diff Temp: {diff_temp}°C")
            ...
            heat_mos_state = d[3]
            fan_mos_state = d[4]
            print(f"    MOS States - Charge: {chg_mos_state}  Discharge: {dischg_mos_state}  Precharge: {pre_mos_state}  Heat: {heat_mos_state}  Fan: {fan_mos_state}")
            ...
            DO_state = d[3]
            DI_state = d[4]
            print(f"    Battery State: {bat_state}  Charge Detect: {chg_detect}  Load Detect: {load_detect}  DO State: {DO_state}  DI State: {DI_state} (Might be wrong)")
...
def main():
    if len(sys.argv) < 2:
        print(f"Usage: {sys.argv[0]} <candump_logfile>")
        sys.exit(1)
